    ...
    const row = (message?: Message) => {
        ...
        return (
            <div key={chatId} onClick={()=> router.push(`/chat/${chatId}`)} className='flex p-5 items-center space-x-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-700' >
                <UserAvatar name={message?.user.name ? message?.user.name : session?.user.name} image={message?.user.image || session?.user.image} />
                <div className='flex-1'>
                    <p className='font-bold'>
                        {!message && "New Chat "}
                        {[message?.user.name].toString().split(" ")[0]}
                    </p>
                ...
                <div className='text-xs text-gray-400 text-right'>
                    <p className='mb-auto'>
                        {message ? new Date(message.timestamp).toLocaleDateString() : "No Message Yet..."}
                    </p>
                    <p>chat #{prettyUUID()}</p>
                </div>
            </div>
